...
import { InboundModelDto } from 'kafka/common/validation/inboundModelSchema';
...
export class ConsumerService {
  ...
  async incomingMessage(
    ...
    switch (message.msgType) {
      case "RET_DHLE_DUTYDRAWBACK_GLOBALE_DATAGEN":
  ...
  private async parseMessage(message: any): Promise<void> {
    //console.log("message", message.transformedMessage);
    const encodedMessage = message.transformedMessage[0];
    const decodedMessage = JSON.parse(
      Buffer.from(encodedMessage, 'base64')
        .toString('utf-8')
    );

    //if the message is in encypted format
    switch (decodedMessage.messageType) {
      case "IMPORT_DUTY_OB":
        console.log("parsed message", decodedMessage.messageType);
        await this.commandBus.execute(
          new CreateOutboundCommand(
            decodedMessage as OutboundModel
          ),
        );
        //await this.kafkaProducerService.post(parsedMessage,transformerTopic);  //to transformer
        break;
      case "IMPORT_DUTY_IB":
        ...
        await this.kafkaProducerService.postBless(JSON.stringify(decodedMessage), "d")
          .then((res: any) => {
